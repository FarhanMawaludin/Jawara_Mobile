...
class _AspirationListSectionState extends ConsumerState<AspirationListSection> {
  ...
  Widget build(BuildContext context) {
    // For now, ignore wargaId filter until backend method is ready
    // TODO: Implement aspirationByWargaProvider when getAspirationsByWarga is available
    final asyncList = ref.watch(aspirationListProvider);

    return asyncList.when(
      data: (items) {
        // Apply wargaId filter client-side if needed
        var filtered = items;
        // if (widget.wargaId != null) {
        //   filtered = items.where((e) => e.wargaId == widget.wargaId).toList();
        // }
